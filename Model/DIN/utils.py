...
def create_amazon_electronic_dataset(file, embed_dim=8, maxlen=40):
    ...
    for user_id, hist in tqdm(reviews_df.groupby('user_id')):
        ...
        for i in range(1, len(pos_list)):
            ...
            hist_i = hist.copy()
            #每一个用户的最后一次作为test，倒数第二次作为val，剩下作为train
            if i == len(pos_list) - 1:
    ...
    # 在这 把历史行为拉成一个maxlen长的list 如果大于maxlen 则从后往前取maxlen个  如  pad_sequences([[1,2]], maxlen=5) => [[0,0,0,1,2]]
    print('==================Padding===================')
    train_X = [np.array([0.] * len(train)), np.array([0] * len(train)),
               pad_sequences(train['hist'], maxlen=maxlen),
               np.array(train['target_item'].tolist())] # [[N,], [N,], [N,maxlen, 2]] 每个 item只对应一个cate 所以是2  N代表处理后数据总条数
    train_y = train['label'].values                     # [N,]
